...
import { type CreateNote, type EmojiMartData } from "../../types";
...
export default function NoteCreationPage() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    createNote(
      { ...formData, authorId: session.id },
      {
        onSuccess: () => {
          toast.success("La note a été crée avec succès");
          navigate("/notes");
        },
        onError: (error) => {
  ...
  return (
    <>
      <title>Création de note - QG10banc</title>
